...
void Snake::JumpOn()
{
	// Queue a jump to be consumed on the next move
	pendingJump = true;
}

void Snake::JumpOff()
{
	// No longer used - jump is auto-consumed after move
	// Kept for backwards compatibility but does nothing
}

void Snake::ApplyPendingJump()
{
	// Apply pending jump for this move cycle, then reset
	if (pendingJump)
	{
...
Location Snake::GetNextHeadLocation(const Location& delta, Board& brd) const
{
	...
	// Wrap X coordinate (handle multi-tile jumps)
	while (new_loc.x < 0)
	{
	...
	// Wrap Y coordinate (handle multi-tile jumps)
	while (new_loc.y < 0)
	{
